...
var filePath = "res://objects/staticobjects/_middle-east/city/city_architecture/gas_station/gas_station.con"
...
func importCollisionMesh(filePath, root, objectData):
	...
	for node in colNode.get_children():
		...
		## Code here to scan through the materials, match them with the physics material
		## loop through the ids, split the name col1_0_col <-- 
		## Match that with the materials in ObjectData
		## Then, retreive the ID and name
		## rename the colstaticbodynode to include the material name and ID in it str(id)
		## Get the materials and get_id_by material (just loop through to find it object.keys())
